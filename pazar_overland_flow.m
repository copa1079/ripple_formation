...
oscillations = 1.8;                   % chooses the number of storms
% minimum at 6 and at the edges
period       = t_max/oscillations;    % chooses the period of the storms
amp          = 1;                     % chooses the size of the storms
amplitude    = initial*amp;           % actual amplitude...
% set up the precipitation, P, while updating its values
shift            = -1;
...
for i = 1:imax
    ...
    % equals the flux downslope plus the
    % rain minus the infiltraion into the ground.
    %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    dHdt = -dQdx + ( R - I ); % conservation statement
       ...
       Z = z + H;             % updates the total thickness with topography...too thin to see on a plot
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % set up plots
    if(rem(t(i),tplot)==0) 
...
%  references
%  
%  Anderson, R. S., and Anderson, S. P., 2010, Geomorphology: the mechanics and chemistry of landscapes
%  Cambridge University Press, pages 385–387. ISBN: 978-0-521-51978-6. Print.
